...
#dev.new()
accumulated_data <- result_genres_year[result_genres_year$year >= 2000, ]
names(accumulated_data) <- c('year', genre)
accumulated_data_long <- gather(accumulated_data, genre, count, `(no genres listed)`:Western)
...
library(DBI)
library(RMySQL)
con <- dbConnect(MySQL(),host="127.0.0.1",dbname="test",user="root",password="940720")
dbDisconnect(con)
