...
heatmap(hour_table, Rowv = NA, Colv = NA, cexRow = 1.5, cexCol = 1.5, xlab = 'Time Quantile', ylab = 'Time Span', main = "The rush hour of users' comments")
```
...
barplot(as.matrix(users_age)[,2], names.arg = ages, main = 'The distribution of user ages', xlab = 'Age', ylab = 'The number of users', beside = T)
```
...
p + geom_point() + labs(title = 'The scatter plot of ratings of different gender')
```
...
text(Action_ordered_first$`mean(rating)`, (Action_ordered_first$n)+50, Action_ordered_first$title, cex = 0.5, col = "red")
text(Action_ordered_last$`mean(rating)`, (Action_ordered_last$n)+50, Action_ordered_last$title, cex = 0.5, col = "blue")
```
