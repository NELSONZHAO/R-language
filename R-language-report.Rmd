...
#ratings_t <- read.csv('ratings.csv', header = T, stringsAsFactors = F)
```
...
avg_ratings_year <- ratings_year[ratings_year$year > 1990,]$n / year_num[year_num$year > 1990,]$n
...
axis(1, at = 1990:2000, las = 1)
par(new = T)
plot(ratings_year[ratings_year$year > 1990,], type = 'b', yaxt = 'n', ylab = NA , pch = 15, col = rainbow_hcl(2)[2])
...
p + geom_point()
# 绘制电影评分分布
qplot(`mean(rating)`, data = ratings_by_title_50, binwidth = 0.01,fill = type)
```

```{r}
# 发现相似用户（用python计算用户相似度数据集,并存入mysql，再用R读入数据）
library(DBI)
library(RMySQL)
#con <- dbConnect(MySQL(),host="127.0.0.1",dbname="R_report",user="root",password="940720")
# 写movie_ratings表
# dbWriteTable(con, "movie_ratings", movie_ratings)
mrdb <- movie_ratings[, c(1, 2, 23, 24)]


#dbWriteTable(con, "mrdb", mrdb)
#dbDisconnect(con)
# 构建推荐系统-基于用户行为的物品协同过滤
